...
async function address(port:number) {
  try { const ip = await worker('/v1/public-ip'); if (ip?.ip) return `${ip.ip}:${port}`; } catch {}
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ ...s, status: workerState?.status || s.status, address: await address(s.port), worker: workerState });
}
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const data = {
      name: b.name ?? s.name, motd: b.motd ?? s.motd, version: b.version ?? s.version, type: b.type ?? s.type,
      ramMb: Math.min(Math.max(Number(b.ramMb ?? s.ramMb),1024),8192), maxPlayers: Math.min(Math.max(Number(b.maxPlayers ?? s.maxPlayers),1),100),
      ...
      hardcore: b.hardcore ?? s.hardcore, pvp: b.pvp ?? s.pvp, onlineMode: b.onlineMode ?? s.onlineMode, whitelist: b.whitelist ?? s.whitelist,
    ...
    return NextResponse.json({ ...updated, address: await address(s.port) });
  } catch (e) { return NextResponse.json({ error:e instanceof Error?e.message:'Could not update server' },{status:500}); }
}
