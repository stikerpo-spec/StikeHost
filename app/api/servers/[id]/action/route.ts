...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  const ownerId = await getSession(); const { id } = await params;
  ...
  try {
    ...
    const status = result.status === 'running' ? 'running' : (result.status === 'stopped' ? 'stopped' : action === 'stop' ? 'stopped' : 'running');
    const updated = await db.server.update({ where: { id }, data: { status } });
